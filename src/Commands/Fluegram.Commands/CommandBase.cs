...
public abstract class CommandBase<TEntity, TArguments> : ICommand<EntityContext<TEntity>, TEntity, TArguments> where TEntity : class where TArguments : class, new()
{
    protected CommandBase(string id)
    {
        Id = id;
    ...
    public abstract Task ProcessInvalidArgumentsAsync(EntityContext<TEntity> entityContext, IEnumerable<ICommandArgumentParseError> argumentsParseErrors, CancellationToken cancellationToken);
}