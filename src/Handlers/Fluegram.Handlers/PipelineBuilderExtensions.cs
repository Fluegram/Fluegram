...
public static class PipelineBuilderExtensions
{
    public static IPipelineBuilder<TEntityContext, TEntity> UseHandlers<TEntityContext, TEntity>(
        this IPipelineBuilder<TEntityContext, TEntity> pipelineBuilder, Action<HandlersConfigurator<TEntityContext, TEntity>> configureHandlers) where TEntityContext : IEntityContext<TEntity>
        where TEntity : class
    {
        ...
        where TEntityContext : IEntityContext<TEntity> where TEntity : class
    {
            ...
            where THandler : class, IHandler<TEntityContext, TEntity>
        {
            ...
            var filters = Filters.TypeExtensions.FindFiltersFor<TEntityContext, TEntity, THandler>().ToList();
            
            var preProcessingActions = Fluegram.Actions.TypeExtensions.FindPreProcessingActionsFor<TEntityContext, TEntity, THandler>().ToList();
            var postProcessingActions = Fluegram.Actions.TypeExtensions.FindPostProcessingActionsFor<TEntityContext, TEntity, THandler>().ToList();
