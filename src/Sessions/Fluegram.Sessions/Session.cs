    ...
    where TEntityContext : IEntityContext<TEntity> where TEntity : class
{
    ...
    public async Task WriteEntityAsync(TEntity entity, CancellationToken cancellationToken)
    {
        await _entityWriter.WriteAsync(entity, cancellationToken).ConfigureAwait(false);
    }

    public async Task<TEntity?> RequestAsync(TEntityContext entityContext,
        ISessionRequestOptions<TEntityContext, TEntity> options, CancellationToken cancellationToken = default)
    {
               ...
               && !cancellationToken.IsCancellationRequested)
        {
            ...
            var entity = await _entityReader.ReadAsync(cancellationToken);
    ...
    public Task<TEntity?> RequestAsync(TEntityContext entityContext,
        Action<ISessionRequestOptionsBuilder<TEntityContext, TEntity>> configureRequest,
        CancellationToken cancellationToken = default)
    {
        ISessionRequestOptionsBuilder<TEntityContext, TEntity> optionsBuilder =
            new SessionRequestOptionsBuilder<TEntityContext, TEntity>();
