...
public class CounterWidget : WidgetBase<Message, CounterState>
{
    ...
    public override async Task OnInitializedAsync(EntityContext<Message> entityContext,
        CancellationToken cancellationToken)
    {
        _sentMessage =
            await entityContext.InvokeAsync<SendTextMessageParameters, Message>(
                _ => _.UseText("Initiaizing counter widget"), cancellationToken);
    }

    public override async Task OnUpdatedAsync(EntityContext<Message> entityContext, CounterState state,
        CancellationToken cancellationToken)
    {
        await entityContext.InvokeAsync<EditMessageTextParameters, Message>(_ => _
            .UseMessageId(_sentMessage.MessageId)
            .UseText($"Counter: {state.Counter}"), cancellationToken);
    }
}